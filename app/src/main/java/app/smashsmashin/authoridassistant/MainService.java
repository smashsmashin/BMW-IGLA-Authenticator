...
public class MainService extends Service {
    ...
    private void deactivateKeyFob() {
        Log.d(LOG_TAG, "Deactivating Key FOB by unchecking toggle.");
        NotificationManager notificationManager = (NotificationManager) getSystemService(Context.NOTIFICATION_SERVICE);
        if (notificationManager != null) {
            StatusBarNotification[] activeNotifications = notificationManager.getActiveNotifications();
            for (StatusBarNotification sbn : activeNotifications) {
                if (sbn.getPackageName().equals("com.dma.author.authorid") && sbn.getNotification().extras.containsKey("android.title")) {
                    String title = sbn.getNotification().extras.getString("android.title");
                    if (title != null && title.contains("Key Fob Enabled")) {
                        try {
                            sbn.getNotification().actions[0].actionIntent.send();
                            Log.d(LOG_TAG, "Toggle unchecked successfully.");
                        } catch (PendingIntent.CanceledException e) {
                            Log.e(LOG_TAG, "Failed to uncheck toggle", e);
                        }
                        return;
                    }
                }
        ...
        Log.d(LOG_TAG, "No active Key Fob notification found.");
    }
